...
class VerTodasSeries extends Component {
  ...
  componentDidMount() {
    fetch(
      "https://api.themoviedb.org/3/tv/popular?api_key=7e2125641ec3ddbc6ebddb7479ee611c&language=es-ES&page=1"
    )
  ...
  pasarPagina() {
    if (this.state.numeroPagina !== this.state.totalPaginas) {
      fetch(
        `https://api.themoviedb.org/3/tv/popular?api_key=7e2125641ec3ddbc6ebddb7479ee611c&language=es-ES&page=${
          this.state.numeroPagina + 1
        }`
      )
        .then((response) => response.json())
        .then((data) => {
  ...
  render() {
    return (
      <>
        <Header />
        <FormBusqueda />
        <main className="main-ver-todas-las-series">
        <h1> Series </h1>
          <div className="series-container">
            {this.state.series.map((serie, index) => (
              <Card
                ...
                mostrarBotonFavoritos={true} // Mostrar botón Agregar/Quitar favoritos
                mostrarEnlaceVerDetalles={true} // Mostrar enlace Ver detalles
              />
            ))}
          </div>
          <button
            className="button-cargar-series"
            onClick={() => this.pasarPagina()}
          >
            Cargar más series
          </button>
        </main>
